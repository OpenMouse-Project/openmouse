    ...
    <div class="control-shell is-empty">
      ...
      <main class="control-panel">
        ...
        <section class="settings-grid device-data" aria-label="Mouse status">
          ...
          <article id="game-mode-card" class="setting-card" data-pending-key="game-mode-enabled game-mode-idle-rate game-mode-gaming-rate" hidden><div class="setting-heading"><div><p>GAME MODE</p><h2>Switch polling rate automatically</h2></div><button id="game-mode-toggle" class="${TOGGLE}" type="button" role="switch" aria-checked="false" disabled>Off</button></div><div id="game-mode-controls" hidden><label class="${FIELD_LABEL}">Idle rate<select id="game-mode-idle-rate"></select></label><label class="${FIELD_LABEL} spaced">Gaming rate<select id="game-mode-gaming-rate"></select></label></div><div id="game-mode-install" class="game-mode-install"><p class="setting-note">Game Mode needs the OpenMouse Companion app running in the background to switch your polling rate the instant a game launches, even if this tab is closed.</p><ol class="game-mode-steps"><li><strong>Download and install.</strong> <a id="game-mode-download" href="https://github.com/xBambooz/OpenMouseCompanion/releases/latest" target="_blank" rel="noopener">Download OpenMouse Companion</a>, run the installer, then launch it.</li><li><strong>Allow this site.</strong> Your browser and the Companion app will each ask for permission the first time. Choose Allow on both.</li></ol></div><small id="game-mode-status" class="setting-note">Companion: not connected.</small></article>
