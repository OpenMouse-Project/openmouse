...
/** 2024-01-07 was a Sunday; used as the anchor for locale weekday names. */
function weekdayName(tag: string, index: number): string {
  return new Date(2024, 0, 7 + index).toLocaleDateString(tag, { weekday: "long" });
}

function weekdayNarrow(tag: string): string[] {
  return Array.from(
    { length: 7 },
    (_, index) => new Date(2024, 0, 7 + index).toLocaleDateString(tag, { weekday: "narrow" }),
  );
}

function formatDay(day: string, tag: string) {
...
/* ---------- login ---------- */
function LoginForm({ onLoggedIn, locale }: { onLoggedIn: () => void; locale: InterfaceLocale }) {
  ...
  const submit = async (event: Event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(body.message ?? t(locale, "adm.loginFailed"));
        return;
      }
      onLoggedIn();
    } catch {
      setError(t(locale, "adm.networkError"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <form onSubmit={submit} className="adm-login">
      <h1>{t(locale, "adm.title")}</h1>
      <input
        type="password"
        autoFocus
        placeholder={t(locale, "adm.password")}
        value={password}
        onInput={(e) => setPassword((e.target as HTMLInputElement).value)}
        className="adm-login-input"
      />
      <button type="submit" disabled={busy || !password} className="adm-login-button">
        {busy ? t(locale, "adm.checking") : t(locale, "adm.signin")}
      </button>
...
/* ---------- chart ---------- */
function DailyChart({ points, locale, tag }: { points: DailyPoint[]; locale: InterfaceLocale; tag: string }) {
  const [hover, setHover] = useState<number | null>(null);
  const width = 640;
  const height = 168;
  const max = Math.max(1, ...points.map((p) => p.view_count));
  if (!points.length) return <p className="adm-muted">{t(locale, "adm.noHistory")}</p>;
  ...
  return (
    <div>
      <div className="adm-chart-focus">
        ...
        <span>{t(locale, "adm.viewsWord")} · {formatDay(active.day, tag)} · {tp(locale, "adm.peakConcurrent", { n: active.peak_concurrent })}</span>
      </div>
      ...
      <div className="adm-axis-row">
        <span className="adm-axis-label">{formatDay(points[0].day, tag).toUpperCase()}</span>
        <span className="adm-axis-label">{formatDay(points[points.length - 1].day, tag).toUpperCase()}</span>
      </div>
    </div>
  );
}
...
function WeekdayHeatmap({ points, locale, tag }: { points: DailyPoint[]; locale: InterfaceLocale; tag: string }) {
  const weeks = useMemo(() => {
  ...
  return (
    <div>
      <div className="adm-heatmap">
        {weeks.map((week, wi) => (
          <div className="adm-heatmap-row" key={wi}>
            ...
            {week.map((cell, di) =>
              cell ? (
                <div key={di} className="adm-hm-cell" title={`${formatDay(cell.day, tag)}: ${cell.view_count} ${t(locale, "adm.viewsWord")}`} style={{ background: `rgba(57, 226, 160, ${(0.12 + (cell.view_count / max) * 0.7).toFixed(2)})` }} />
              ) : (
      ...
      <div className="adm-hm-foot">
        <span />
        {weekdayNarrow(tag).map((l, i) => <span key={i}>{l}</span>)}
      </div>
    </div>
  );
}

function DataTable({ rows, labelKey, valueKey, nameHeader, valueHeader, locale }: {
  rows: CountRow[]; labelKey: "country" | "mouse_model"; valueKey: "view_count" | "uses"; nameHeader: string; valueHeader: string;
  locale: InterfaceLocale;
}) {
  const max = Math.max(1, ...rows.map((r) => Number(r[valueKey]) || 0));
  if (!rows.length) return <p className="adm-muted">{t(locale, "adm.noData")}</p>;
  return (
            ...
            <tr key={String(row[labelKey])}>
              ...
              <td>
                <div className="adm-table-name">
                  ...
                  {row[labelKey] || t(locale, "adm.unknown")}
                </div>
              </td>
...
function Dashboard() {
  ...
  const load = async (days: number) => {
    ...
    if (!response.ok) { setError(t(locale, "adm.loadFail")); return; }
  ...
  const bestWeekday = useMemo(() => {
    ...
    return weekdayName(tag, bestIndex);
  }, [daily]);
  ...
  const trackedSince = daily.length ? formatDay(daily[0].day, tag) : "—";

  if (error === "session-expired") return <LoginForm onLoggedIn={() => load(range)} locale={locale} />;
    ...
    <div className="adm-root">
      <div className="adm-topbar">
        <div className="adm-brand">
          ...
          <div>
            <div className="adm-brand-name">{t(locale, "adm.title")}</div>
            <div className="adm-brand-sub">{lastUpdated ? tp(locale, "adm.updated", { time: lastUpdated.toLocaleTimeString(tag) }) : t(locale, "adm.loading")}</div>
          </div>
        </div>
        <div className="adm-topbar-right">
          ...
          <button className="adm-icon-btn" title={t(locale, "adm.signout")} onClick={() => fetch("/api/admin/logout", { method: "POST" }).then(() => setError("session-expired"))}>
            <IconLogout />
          </button>
        </div>
      </div>
      ...
      <div className="adm-content">
        ...
        <div className="adm-bento">
          <div className="adm-tile adm-live-tile a-live">
            <div className="adm-live-label"><span className="adm-pulse" />{t(locale, "adm.live")}</div>
            <div className="adm-live-value">{stats?.live ?? "—"}</div>
            <div className="adm-live-sub">{t(locale, "adm.peak")} <b>{stats?.allTimePeak ?? "—"}</b>{stats?.allTimePeakAt ? ` · ${new Date(stats.allTimePeakAt).toLocaleDateString(tag)}` : ""}</div>
            <div className="adm-live-ring">
              <div className="adm-ring-track"><div className="adm-ring-fill" style={{ width: `${concurrencyRatio}%` }} /></div>
              <div className="adm-ring-label">{tp(locale, "adm.ofPeak", { n: concurrencyRatio.toFixed(0) })}</div>
            </div>
          </div>

          <div className="adm-tile adm-chart-tile a-chart">
            <div className="adm-chart-head">
              <p className="adm-chart-title">{t(locale, "adm.dailyViews")}</p>
              <span className="adm-chart-sub">{tp(locale, "adm.lastAvg", { n: daily.length, avg: avgDaily.toFixed(1) })}</span>
            </div>
            <DailyChart points={daily} locale={locale} tag={tag} />
          </div>

          <div className="adm-tile adm-mini a-peak">
            <div className="adm-mini-label"><IconTrend />{tp(locale, "adm.viewsDays", { n: daily.length })}</div>
          ...
          <div className="adm-tile adm-mini a-avg">
            <div className="adm-mini-label"><IconCalendar />{t(locale, "adm.avgDay")}</div>
            <div className="adm-mini-value indigo">{avgDaily.toFixed(1)}</div>
            <div className="adm-mini-sub">{t(locale, "adm.medianAdj")}</div>
          </div>
          <div className="adm-tile adm-mini a-best">
            <div className="adm-mini-label"><IconStar />{t(locale, "adm.bestDay")}</div>
            <div className="adm-mini-value">{peakDay?.view_count ?? "—"}</div>
            <div className="adm-mini-sub">{peakDay ? formatDay(peakDay.day, tag) : "—"}</div>
          </div>
          <div className="adm-tile adm-mini a-trend">
            <div className="adm-mini-label"><IconTrend />{t(locale, "adm.bestWeekday")}</div>
            <div className="adm-mini-value indigo">{bestWeekday ?? "—"}</div>
            <div className="adm-mini-sub">{t(locale, "adm.byViews")}</div>
          </div>

          <div className="adm-tile adm-table-tile a-regions">
            <p className="adm-chart-title">{t(locale, "adm.regions")}</p>
            <span className="adm-chart-sub">{tp(locale, "adm.regionsSub", { views: regionTotal.toLocaleString(), n: regions.length })}</span>
            <DataTable rows={regions} labelKey="country" valueKey="view_count" nameHeader={t(locale, "adm.country")} valueHeader={t(locale, "adm.views")} locale={locale} />
          </div>
          <div className="adm-tile adm-table-tile a-mice">
            <p className="adm-chart-title">{t(locale, "adm.mice")}</p>
            <span className="adm-chart-sub">{tp(locale, "adm.miceSub", { n: mouseTotal.toLocaleString(), m: mice.length })}</span>
            <DataTable rows={mice} labelKey="mouse_model" valueKey="uses" nameHeader={t(locale, "adm.model")} valueHeader={t(locale, "adm.uses")} locale={locale} />
          </div>

          <div className="adm-tile a-heat">
            <p className="adm-chart-title">{t(locale, "adm.traffic")}</p>
            <span className="adm-chart-sub">{t(locale, "adm.trafficSub")}</span>
            <WeekdayHeatmap points={daily} locale={locale} tag={tag} />
          </div>
          <div className="adm-tile a-detail">
            <p className="adm-chart-title">{t(locale, "adm.session")}</p>
            <span className="adm-chart-sub">{t(locale, "adm.sessionSub")}</span>
            <div className="adm-kv-grid">
              <div className="adm-kv"><div className="adm-kv-label">{t(locale, "adm.trackedSince")}</div><div className="adm-kv-value">{trackedSince}</div></div>
              <div className="adm-kv"><div className="adm-kv-label">{t(locale, "adm.daysTracked")}</div><div className="adm-kv-value">{daily.length}</div></div>
              <div className="adm-kv"><div className="adm-kv-label">{t(locale, "adm.quietest")}</div><div className="adm-kv-value">{quietDay ? `${quietDay.view_count} · ${formatDay(quietDay.day, tag)}` : "—"}</div></div>
              <div className="adm-kv"><div className="adm-kv-label">{t(locale, "adm.trend7d")}</div><div className="adm-kv-value">{weekTrend != null ? `${weekTrend >= 0 ? "+" : ""}${weekTrend.toFixed(1)}%` : "—"}</div></div>
            </div>
          </div>
        </div>
      </div>
